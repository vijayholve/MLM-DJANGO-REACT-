...
class MLMUserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        sponsor = None
        print(f"Sponsor code received:2 {sponsor_code}")  # Debugging line

        if sponsor_code:
            try:
                def_sponcer=MLMUser.objects.get(username=sponsor_code)
                sponsor = MLMUser.objects.get(custom_sponsor_id=def_sponcer)
