...
class MLMUserSerializer(serializers.ModelSerializer):
    ...
    passwordConfirmation = serializers.CharField(write_only=True)  # Accept but not save

    class Meta:
        model = MLMUser
        fields = ['username', 'email', 'password', 'sponsor_id', 'passwordConfirmation']
        extra_kwargs = {
    ...
    def validate_sponsor_id(self, value):
        if value:
            try:
                sponsor = MLMUser.objects.get(sponsor_id=value)
                return sponsor
