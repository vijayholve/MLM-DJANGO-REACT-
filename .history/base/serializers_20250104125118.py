...
class MLMUserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        print(f"Sponsor code received: {sponsor_code}")   
        print(f"Validated data: {validated_data}")  # Check the whole payload

        sponsor = None

        if sponsor_code:
            try:
                sponsor = MLMUser.objects.get(sponsor_id=sponsor_code)
                print(f"Sponsor found: {sponsor}")
            except MLMUser.DoesNotExist:
                raise serializers.ValidationError("Sponsor not found.")
                # Handle exception (e.g., raise a validation error or return a custom message)
        else:
